...
images: 'https://pbs.twimg.com/media/G9qLjHBXsAAKh4r.jpg'
---
...
That's what x402 v2 is responding to. v1 proved the idea. v2 hardens it for production.

This post isn't a spec summary. It's a builder summary, grounded in working code from a private inference demo and an accompanying facilitator reference implementation.
...
v2 adopts CAIP-2 (Chain Agnostic Improvement Proposal 2), which uses the format "namespace:reference". In the facilitator reference implementation, Arbitrum One and Arbitrum Sepolia are represented explicitly as eip155:42161 and eip155:421614. Anything else is treated as compatibility glue or rejected outright.
...
The facilitator reference implementation exposes this via GET /supported.

The extensions array is where v2's formal extensions system lives. Extensions handle optional features that operate independently of core payment mechanics, each with its own info object and JSON Schema definition. The demo doesn't use any yet, but the capability is there for things like automatic discovery and indexing.

The private inference demo uses the /supported endpoint directly. Before attempting a v2 payment flow, it queries /supported, filters for v2 kinds, and confirms that the target Arbitrum network is supported.
...
The private inference demo relies on header-first behavior directly, treating payment negotiation as protocol metadata rather than application payload.
